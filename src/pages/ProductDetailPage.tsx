		...
		return (
			<div className="pt-32 text-center">
				<div className="max-w-md mx-auto px-4">
					...
					<p className="text-volcanic-400 mb-8">{t("product.not_found.description")}</p>
					<Link
						to="/products"
				...
				{checkoutStatus === "success" && (
					<div className="mb-6 flex items-center gap-3 p-4 bg-ark-600/10 border border-ark-600/20 rounded-xl animate-fade-in">
						...
						<button
							...
							className="text-volcanic-400 hover:text-heading transition-colors"
						>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
					...
					<div className="space-y-6">
						<div className="flex flex-wrap gap-2">
							...
							{product.featured && (
								<Badge variant="featured">{t("product.badge.featured")}</Badge>
							)}
							{stockTracked &&
								(outOfStock ? (
						...
						<h1 className="text-3xl lg:text-4xl font-bold text-heading">
							{product.name}
						</h1>
						...
						<div className="space-y-2">
							<div className="flex items-baseline gap-3">
								...
								{product.subscription && product.duration_periodicity && (
									<span className="text-volcanic-400">
										/{" "}
										{product.period_num && product.period_num > 1
											? `${product.period_num} `
											: ""}
							...
							{extrasPrice > 0 && (
								<div className="text-sm text-volcanic-400">
									{t("common.base_price_prefix")} {product.price.toFixed(2)} ${" "}
									{t("common.plus_options")} {extrasPrice.toFixed(2)} $
								</div>
							)}
							...
							<div className="glass-card p-4 flex items-center gap-3 group hover:border-ark-600/30 transition-all duration-200">
								...
								<div>
									<div className="text-sm font-medium text-heading">
										{t("product.feature.instant")}
									</div>
									<div className="text-xs text-volcanic-400">
										{t("product.feature.auto_delivery")}
									</div>
								</div>
							</div>
							<div className="glass-card p-4 flex items-center gap-3 group hover:border-ark-600/30 transition-all duration-200">
								...
								<div>
									<div className="text-sm font-medium text-heading">
										{t("product.feature.secure")}
									</div>
									<div className="text-xs text-volcanic-400">
										{t("product.feature.protected_payment")}
									</div>
								</div>
							</div>
							<div className="glass-card p-4 flex items-center gap-3 group hover:border-ark-600/30 transition-all duration-200">
								...
								<button
									...
									onClick={() => {
										...
										if (result.ok && result.replaced) {
											addToast(
												t("product.toast.replaced_by_subscription", {
													name: product.name,
												}),
												"info",
											);
											...
											addToast(
												t("product.toast.added_subscription", {
													name: product.name,
												}),
												"success",
											);
								...
								<button
									...
									onClick={() => {
										...
										if (stockTracked && quantity > stockValue) {
											addToast(
												t("product.toast.max_stock", { qty: stockValue }),
												"warning",
											);
